...
# -----------------------------
def load_data():
    flood_log_file = "dire_dawa_flood_log.csv"
    river_obs_file = "river.csv"
    river_pred_file = "river_volume_prediction.csv"

    # --- Flood log ---
    if os.path.exists(flood_log_file):
        flood_df = pd.read_csv(flood_log_file)
        if "datetime" in flood_df.columns:
            flood_df["datetime"] = pd.to_datetime(flood_df["datetime"], errors="coerce")
    else:
        # Generate realistic 30-day dummy flood/weather data
        dates = pd.date_range(end=datetime.today(), periods=30)
        flood_df = pd.DataFrame({
            "datetime": dates,
            "flood_index_mean": np.clip(np.random.normal(0.4, 0.2, size=30), 0, 1),
            "current_precip": np.random.randint(0, 50, size=30),
            "forecast_precip": np.random.randint(0, 50, size=30)
        })
    ...
    if os.path.exists(river_obs_file):
        ...
        river_obs_df = pd.DataFrame({
            ...
            "discharge_estimated": np.random.randint(50, 300, size=3)
        })
    ...
    if os.path.exists(river_pred_file):
        ...
        river_pred_df = pd.DataFrame({
            "polygon_name": ["River A", "River B", "River C"],
            "discharge_predicted": np.random.randint(50, 300, size=3)
        })
...
def update_graphs(selected_metric):
    ...
    time_chart = px.line(flood_df, x="datetime", y=selected_metric, title=selected_metric.replace("_"," ").title()) if not flood_df.empty else go.Figure()
...
if __name__ == "__main__":
    ...
    app.run_server(host="0.0.0.0", port=port, debug=False)        river_df_obs = pd.DataFrame({
            'polygon_name': ['River A', 'River B', 'River C'],
...
latest_risk = flood_df['flood_index_mean'].iloc[-1] if not flood_df.empty else 0
if latest_risk < 0.3:
    alert_color, status, message = "success", "SAFE – No Flood Risk", "It is fine today. Enjoy your day safely!"
elif latest_risk < 0.6:
    alert_color, status, message = "info", "MONITOR – Low Risk", "Some rain possible, but currently safe."
elif latest_risk < 0.8:
    alert_color, status, message = "warning", "WARNING – Medium Risk", "Possible flood soon. Stay alert!"
else:
    alert_color, status, message = "danger", "HIGH RISK – Flood Likely!", "Flood expected! Stay safe and avoid risk areas!"
...
app.layout = dbc.Container([
    ...
    html.Div("No data available yet — CSVs missing!" if flood_df.empty else "", className="text-center text-warning mb-4"),
